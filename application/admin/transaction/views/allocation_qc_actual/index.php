...
modal_body();
    form_open(base_url('transaction/allocation_qc_actual/import'),'post','form-import');
        col_init(3,9);
...
function getData() {
		...
		var page = base_url + 'transaction/allocation_qc_actual/data';
...
function save_perubahan() {
	...
	$.ajax({
		url : base_url + 'transaction/allocation_qc_actual/save_perubahan',
		data 	: {
			'json' : jsonString,
			verifikasi : i
		},
...
function lanjut() {
	$.ajax({
		url : base_url + 'transaction/allocation_qc_actual/proses_rounding/'+tahun,
			data : {id:id_proses,tahun : tahun, bulan : $('#bulan').val()},
			type : 'post',
			dataType : 'json',
