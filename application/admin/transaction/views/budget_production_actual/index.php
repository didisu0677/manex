		...
		<div class="float-right">
			...
			<select class="select2 infinity custom-select" style="width: 80px;" id="filter_tahun">
				<?php foreach ($tahun as $tahun) { ?>
					<option value="<?php echo $tahun->tahun -1; ?>" <?php if ($tahun->tahun-1 == user('tahun_budget') -1) echo ' selected'; ?>><?php echo $tahun->tahun; ?></option>
				<?php } ?>
			</select>
...
modal_body();
	form_open(base_url('transaction/budget_production_actual/import'),'post','form-import');
		col_init(3,9);
...
<script type="text/javascript">
    ...
    function getData() {
        ...
        var page = base_url + 'transaction/budget_production_actual/data';
	...
	function save_perubahan() {
		...
		$.ajax({
			url: base_url + 'transaction/budget_production_actual/save_perubahan',
			data: {
				'json': jsonString,
	...
	$(document).on('click', '.btn-export', function() {
		...
		table += '<tr><td colspan="1">' + judul + ' Volume Produksi </td></tr>';
	...
	function download_template(){
		let tahun = $('#tahun').val();
		window.open(base_url + 'transaction/budget_production_actual/template?tahun='+tahun)
	}
	...
	// function do_import(){
	// 	$.ajax({
	// 		url: base_url + 'transaction/budget_production_actual/import',
	// 		data: {
	// 			tahun: $('#tahun').val(),
	...
	function refresh_page() {
        $(document).on('click', '.swal-button--confirm', function(){
            setTimeout(function () {
                window.location.href = '<?php echo site_url('transaction/budget_production_actual') ?>';
            }, 1000);
        })
    }
