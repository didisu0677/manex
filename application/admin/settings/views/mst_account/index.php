...
<div class="content-body">
	...
	table_open('',true);
		thead();
			tr();
				th(lang('account_name'),'','width="300px"');
				th(lang('description'));
				th(lang('account_code'),'text-center','width="120px"');
...
<?php 
	modal_open('modal-form','','modal-lg');
		modal_body();
			form_open(base_url('settings/mst_account/save'),'post','form');
				col_init(3,9);
				input('hidden','id','id');
				select2(lang('sub_dari'),'parent_id');
				input('text',lang('account_name'),'account_name','required');
...
<script type="text/javascript">
	function getData() {
		...
		$.ajax({
			url 	: base_url + 'settings/mst_account/data',
			data 	: {},
			type	: 'get',
			dataType: 'json',
	...
	$('.btn-sort').click(function(){
		...
		$.ajax({
			url : base_url + 'settings/mst_account/data/sortable',
			type : 'get',
			dataType : 'json',
			success : function(response) {
	...
	$('#save-posisi').click(function(e){
		...
		$.ajax({
			url : base_url + 'settings/mst_account/save_sortable',
			type : 'post',
			data : serialized,
			dataType : 'json',
			success : function(response) {
