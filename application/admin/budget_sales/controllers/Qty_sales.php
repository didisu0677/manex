...
class Qty_sales extends BE_Controller {

    var $controller = 'qty_sales';
    ...
    function data($tahun="",$divisi="",$category="",$sector="",$tipe = 'table'){
        ...
        $table = 'tbl_budget_qtysales_' . $tahun; 
        ...
        $response	= array(
            'table'		=> $this->load->view('budget_sales/qty_sales/table',$data,true),
            'table2'		=> $this->load->view('budget_sales/qty_sales/table2',$data,true),
            'table3'		=> $this->load->view('budget_sales/qty_sales/table3',$data,true),
        );
    ...
    function save_perubahan() {       
        ...
        $table = 'tbl_budget_qtysales_'.post('tahun');
        ...
        foreach($data as $id => $record) {
            ...
            foreach ($result as $r => $v) {
                ...
                recalculate_sales(post('tahun'),$upd->budget_product_code,$upd->budget_product_sector);
            }      
        }
    }
    ...
    function template() {
        ...
        $tahun = get('tahun');
        $data = get_data('tbl_budget_qtysales_'.$tahun)->result_array();
        ...
        $config = [
            'title' => 'template budget qtysales',
            'header' => $arr,
