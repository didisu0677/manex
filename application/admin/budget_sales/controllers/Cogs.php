...
class Cogs extends BE_Controller {
    var $controller = 'cogs';
    ...
    function data($tahun="",$divisi="",$category="",$sector="",$tipe = 'table'){
        ...
        $response	= array(
            'table'		=> $this->load->view('budget_sales/cogs/table',$data,true),
            'table2'		=> $this->load->view('budget_sales/cogs/table2',$data,true),
            'table3'		=> $this->load->view('budget_sales/cogs/table3',$data,true),
        );
    ...
    function proses ($tahun="") {
        ...
        $arr = [
            'select' => 'distinct budget_product_code, budget_product_sector',
            'where' => [
                'tahun' => $tahun,
                // 'budget_product_code' => 'CIHODNSPDM'
            ],
        ];
        
        $lst_product = get_data('tbl_budget_cogs_' . $tahun, $arr)->result();

     
        foreach($lst_product as $p) {
            recalculate_sales($tahun, $p->budget_product_code, $p->budget_product_sector);
        }

        render([
			'status'	=> 'success',
			'message'	=> 'Recalculate COGS has benn succesfuly'
		],'json');	
    }
}