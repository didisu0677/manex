...
<div class="content-header page-data" data-additional="<?= $access_additional ?>">
	<div class="main-container position-relative">
		...
		<div class="float-right">
			...
			if(in_array(user('id_group'),[GROUP_SALES_MARKETING,ADMIN_UTAMA,DEVELOPER])) {
...
<script type="text/javascript">
    ...
    function getSubaccount() {
        ...
        $.ajax({
                url : base_url + 'budget_sales/qty_sales/get_subaccount',
	...
	function getSector() {
		$('#filter_sector').html('');
		$.ajax({
			url: base_url + 'budget_sales/qty_sales/get_sector',
			data: {
    ...
    function getData() {
        ...
        var page = base_url + 'budget_sales/qty_sales/data';
	...
	function save_perubahan() {
		...
		$.ajax({
			url: base_url + 'budget_sales/qty_sales/save_perubahan',
			data: {
				'json': jsonString,
	...
	function download_template(){
		let tahun = $('#tahun').val();
		window.open(base_url + 'budget_sales/qty_sales/template?tahun='+tahun)
	}
	...
	// function do_import(){
	// 	$.ajax({
	// 		url: base_url + 'budget_sales/qty_sales/import',
	// 		data: {
	// 			tahun: $('#tahun').val(),
	...
	function refresh_page() {
        $(document).on('click', '.swal-button--confirm', function(){
            setTimeout(function () {
                window.location.href = '<?php echo site_url('budget_sales/qty_sales') ?>';
            }, 1000);
        })
    }
