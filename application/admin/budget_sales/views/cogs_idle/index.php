...
modal_body();
	form_open(base_url('budget_sales/cogs_idle/import'),'post','form-import');
		col_init(3,9);
...
<script type="text/javascript">
    ...
    function getSubaccount() {
        ...
        $.ajax({
                url : base_url + 'budget_sales/cogs_idle/get_subaccount',
	...
	function getSector() {
		$('#filter_sector').html('');
		$.ajax({
			url: base_url + 'budget_sales/cogs_idle/get_sector',
			data: {
    ...
    function getData() {
        ...
        var page = base_url + 'budget_sales/cogs_idle/data';
	...
	function save_perubahan() {
		...
		$.ajax({
			url: base_url + 'budget_sales/cogs_idle/save_perubahan',
			data: {
				'json': jsonString,
	...
	$(document).on('click', '.btn-export', function() {
		...
		table += '<tr><td colspan="1">' + judul + ' COGS IDLE </td></tr>';
	...
	function download_template(){
		let tahun = $('#tahun').val();
		window.open(base_url + 'budget_sales/cogs_idle/template?tahun='+tahun)
	}
	...
	function do_import(){
		$.ajax({
			url: base_url + 'budget_sales/cogs_idle/import',
			data: {
				tahun: $('#tahun').val(),
	...
	function refresh_page() {
        $(document).on('click', '.swal-button--confirm', function(){
            setTimeout(function () {
                window.location.href = '<?php echo site_url('budget_sales/cogs_idle') ?>';
            }, 1000);
        })
    }
