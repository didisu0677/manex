	...
	<div class="main-container mt-6">
		...
		<div class="card-header pl-3 pr-3">
			<ul class="nav nav-pills card-header-pills">
				...
				<li class="nav-item">
					<a class="nav-link" href="#budget" data-toggle="pill" role="tab" aria-controls="pills-budget" aria-selected="true">Monthly Budget <?php echo user('tahun_budget') - 1 ; ?></a>
				</li>
...
modal_body();
	form_open(base_url('budget_sales/discount/import'),'post','form-import');
		col_init(3,9);
...
<script type="text/javascript">
    ...
    function getSubaccount() {
        ...
        $.ajax({
                url : base_url + 'budget_sales/discount/get_subaccount',
	...
	function getSector() {
		$('#filter_sector').html('');
		$.ajax({
			url: base_url + 'budget_sales/discount/get_sector',
			data: {
    ...
    function getData() {
        ...
        var page = base_url + 'budget_sales/discount/data';
	...
	function save_perubahan() {
		...
		$.ajax({
			url: base_url + 'budget_sales/discount/save_perubahan',
			data: {
				'json': jsonString,
	...
	$(document).on('click', '.btn-export', function() {
		...
		table += '<tr><td colspan="1">' + judul + ' Discount </td></tr>';
	...
	function refresh_page() {
        $(document).on('click', '.swal-button--confirm', function(){
            setTimeout(function () {
                window.location.href = '<?php echo site_url('budget_sales/discount') ?>';
            }, 1000);
        })
    }
